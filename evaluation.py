...
def evaluate(benchmark_language, test_project, verification):
    file_dir = f"~/test/c2rust_test/test_file/{benchmark_language}_transcoder"
    for subdir, _, files in os.walk(f"{file_dir}/"):
        for file in files:
            ...
            if verification == "bolero":
                ...
                if os.path.exists(bolero_target_path):
                    shutil.rmtree(bolero_target_path)
                else:
                    print(f"The directory {bolero_target_path} does not exist.") 
            #########################################################################################
            ###################################### BoundedKANI ######################################
            #########################################################################################
